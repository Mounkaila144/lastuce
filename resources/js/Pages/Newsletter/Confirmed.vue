...
<template>
  ...
  <section class="py-16">
    <div class="mx-auto max-w-2xl px-4 text-center sm:px-6 lg:px-8">
      ...
      <h1 class="mt-4 text-3xl font-bold">Bienvenue dans la newsletter !</h1>
      <p class="mt-2 text-surface-fg-muted">
        Votre inscription est confirmée pour <strong class="text-surface-fg">{{ email }}</strong>.
        Vous recevrez votre première édition très bientôt.
      </p>
      ...
      <p class="mt-6 text-xs text-surface-fg-muted">
        Vous pouvez à tout moment
        <Link :href="unsubscribeUrl" class="underline">vous désabonner</Link>.
      </p>
    </div>
  </section>
</template>
