...
// Route de démo Inertia (S1.1) — vérifie que la stack Vue 3 + TS est opérationnelle.
Route::get('/__inertia-test', function () {
    return Inertia::render('InertiaTest', [
        'message' => 'Hello depuis Inertia + Vue 3 + TypeScript',
        'serverTime' => now()->toIso8601String(),
    ]);
})->name('inertia.test');

// Story S2.2 — Catalogue interne des primitives UI. Accessible uniquement
// hors prod (équivalent d'un Storybook léger).
if (! app()->environment('production')) {
    Route::get('/__ui-kit', fn () => Inertia::render('UiKit'))->name('ui.kit');
}

// Story S3.6 — Sitemap et flux RSS (pas de préfixe locale, point d'entrée unique).
Route::get('/sitemap.xml', [\App\Http\Controllers\SyndicationController::class, 'sitemap'])->name('sitemap');
Route::get('/episodes/rss', [\App\Http\Controllers\SyndicationController::class, 'episodesRss'])->name('rss.episodes');
Route::get('/blog/rss', [\App\Http\Controllers\SyndicationController::class, 'blogRss'])->name('rss.blog');
...
Route::group([
    ...
    // Story S3.5 — page de résultats Inertia agrégée.
    Route::get('/search', [\App\Http\Controllers\SearchController::class, 'index'])->name('search');
    // Suggestions JSON conservées sur le préfixe locale pour le legacy front.
    ...
    Route::group(['prefix' => 'astuces'], function () {
        ...
        // Story S4.1 — middleware security:upload pour rate-limiter / scanner.
        Route::post('/', [AstuceController::class, 'store'])
            ->middleware('security:upload')
            ->name('astuces.store');
    ...
    // Newsletter (Epic 5 — double opt-in Inertia). Le throttle anti-spam est
    // appliqué applicativement par les FormRequest pour rester compatible
    // avec le flow Inertia (le middleware security:* renvoie du JSON 429).
    Route::group(['prefix' => 'newsletter'], function () {
        ...
        Route::match(['GET', 'POST'], '/unsubscribe/{token}', [NewsletterController::class, 'unsubscribe'])
...
// Routes SEO legacy — supplantées par SyndicationController (story S3.6).
// Conservées en redirection 301 le temps que les abonnés migrent.
Route::redirect('/rss.xml', '/episodes/rss', 301)->name('rss.legacy');
...
Route::group([
    ...
    // Dashboard — /admin/dashboard est canonique (story S8.2). /admin
    // redirige vers le dashboard pour rester compatible avec les anciens
    // signets et les redirections logout.
    Route::redirect('/', '/admin/dashboard')->name('admin.root');
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard');
    Route::get('/stats', [DashboardController::class, 'getStats'])->name('stats');
    Route::get('/export', [DashboardController::class, 'export'])->name('export');
    Route::post('/cleanup', [DashboardController::class, 'cleanup'])->name('cleanup');
    
    // Gestion des épisodes (story S8.3). Les routes explicites (export,
    // bulk-action) doivent être déclarées AVANT la resource pour ne pas
    // être shadowées par le pattern {episode}.
    ...
    // Modération des astuces (story S8.4).
    ...
    Route::resource('astuces', AstuceAdminController::class);
    
    // Gestion du blog (story S8.7).
    Route::get('/blog/export', [BlogAdminController::class, 'export'])->name('blog.export');
