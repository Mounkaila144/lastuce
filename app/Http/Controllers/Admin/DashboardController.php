...
{
    /**
     * Afficher le dashboard principal (story S8.2, Inertia).
     * Cache Redis 5 min sur les agrégats coûteux (stats + chart data) ;
     * activité récente et alertes restent live pour rester pertinents.
     */
    public function index(Request $request)
    {
        ...
        $stats = Cache::remember('admin_dashboard_stats', 300, function () {
            ...
            'rejected' => AstucesSoumise::where('status', 'rejete')->count(),
                ...
                'partenariats' => [
                    // Statuses persistés : nouveau / en_cours / accepte / refuse
                    // (cf. App\Models\Partenariat).
                    'total' => Partenariat::count(),
                    'pending' => Partenariat::whereIn('status', ['nouveau', 'en_cours'])->count(),
                    'approved' => Partenariat::where('status', 'accepte')->count(),
                    'rejected' => Partenariat::where('status', 'refuse')->count(),
        ...
        // Graphiques de données (derniers 30 jours), cachés 5 min
        $chartData = Cache::remember('admin_dashboard_chart_30d', 300, fn () => $this->getChartData());
    ...
    private function getAlerts()
    {
        ...
        // Partenariats nouveaux à examiner
        $pendingPartenariats = Partenariat::where('status', 'nouveau')->count();
        if ($pendingPartenariats > 0) {
            $alerts[] = [
                'type' => 'info',
                'icon' => 'handshake',
                'title' => 'Partenariats à examiner',
                'message' => "{$pendingPartenariats} demande(s) de partenariat à examiner",
                'action_url' => route('admin.partenariats.index', ['status' => 'nouveau']),
