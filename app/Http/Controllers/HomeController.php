...
{
    /**
     * Page d'accueil — Inertia/Vue (story S3.2). Toute la donnée nécessaire
     * (héros, derniers épisodes, blog, partenaires, stats) est sérialisée en
     * une seule réponse pour éviter un waterfall côté client.
     */
    public function index(): Response
    {
        $featuredEpisode = Episode::published()
            ->where(function ($q) {
                $q->whereNotNull('youtube_url')->orWhereNotNull('facebook_url');
            })
            ->recent()
            ->first();
        ...
        $latestEpisodes = Episode::published()
            ->recent()
            ->when($featuredEpisode, fn ($q) => $q->where('id', '!=', $featuredEpisode->id))
            ->limit(6)
            ->get();
        ...
        return Inertia::render('Home', [
            ...
            'stats' => [
                'episodes' => Episode::published()->count(),
                'newsletter' => NewsletterAbonne::actif()->count(),
                'astuces' => AstucesSoumise::approuve()->count(),
            ],
        ]);
    }
